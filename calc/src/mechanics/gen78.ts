...
export const strikerdmg: any = 0;
export const magdmg: any = 0;
export function calculateSMSSSV(
  ...
  var defenderIgnoresAbility = defender.hasAbility(
    "Full Metal Body",
    "Neutralizing Gas",
    "Prism Armor",
    "Shadow Shield"
  );
  var attackerIgnoresAbility = attacker.hasAbility(
    "Mold Breaker",
    "Teravolt",
    "Turboblaze"
  );
  ...
  if (
    field.hasTerrain("Fairy Tale") &&
    move.named("Cut", "Sacred Sword", "Secret Sword", "Slash")
  ) {
    type = "Steel";
  }
  if (
    field.hasTerrain("Murkwater") &&
  ...
  var type1Effectiveness = getMoveEffectiveness(
    gen,
    move,
    defender.types[0],
    isGhostRevealed,
  ...
  if (
    ...
    var effectiveness = gen.types.get(toID(move.type))!.effectiveness;
  ...
  if (
    ...
    gen.types.get(toID(move.type))!.effectiveness["Dragon"]! > 1
  ) {
    typeEffectiveness /= 2;
    desc.terrain = field.terrain;
  }
  if (
    field.hasTerrain("Flower Garden 4", "Flower Garden 5") &&
    defender.hasType("Grass") &&
    gen.types.get(toID(move.type))!.effectiveness["Grass"]! > 1
  ) {
    typeEffectiveness /= 2;
    desc.terrain = field.terrain;
  }
  var moveType =
    gen.types.get(toID(move.type))!.effectiveness[defender.types[0]]! *
    (defender.types[1]
      ? gen.types.get(toID(move.type))!.effectiveness[defender.types[1]]!
      : 1);
  var addedType;
  switch (field.terrain) {
    case "Ashen Beach":
      addedType = gen.types.get(toID("psychic"));
    ...
    case "New World":
    case "Dragon's Den":
      addedType = gen.types.get(toID("fire"));
      if (move.named("Doom Desire") && field.hasTerrain("New World")) {
        ...
        move.named("Smack Down", "Thousand Arrows", "Clear Smog", "Smog") &&
    ...
    case "Wasteland":
    case "Corrosive Mist":
      addedType = gen.types.get(toID("poison"))!;
      if (
        (move.named(
          "Mud Bomb",
          "Mud Shot",
        ...
        move.named("Mud Bomb", "Mud-Slap", "Mud Shot") &&
      ...
      if (
        (move.named("Bubble", "Bubble Beam", "Sparkling Aria", "Energy Ball") ||
    ...
    case "Chess Board":
    case "Rocky":
      addedType = gen.types.get(toID("rock"));
      if (
        move.named(
          "AncientPower",
          "Nature Power",
    ...
    case "Crystal Psychic":
      if (
        ...
        addedType = gen.types.get(toID("water"));
        typeEffectiveness =
          moveType *
          addedType!.effectiveness[defender.types[0]]! *
          (defender.types[1]
            ...
            : 1);
        if (
          defender.hasAbility("Water Absorb", "Dry Skin", "Storm Drain") ||
        ...
        addedType = gen.types.get(toID("grass"));
        typeEffectiveness =
          moveType *
          addedType!.effectiveness[defender.types[0]]! *
          (defender.types[1]
      ...
      break;
    case "Electric":
      addedType = gen.types.get(toID("electric"));
      if (
        move.named(
          "Explosion",
          "Hurricane",
        ...
        typeEffectiveness =
          ...
          (defender.types[1]
            ...
            : 1)!;
        if (
          defender.hasAbility("Volt Absorb", "Lightning Rod", "Motor Drive")
        ) {
          typeEffectiveness *= 0;
        }
      }
      break;
    case "Fairy Tale":
      addedType = gen.types.get(toID("dragon"));
    ...
    case "Icy":
    case "Snowy Mt":
      addedType = gen.types.get(toID("ice"));
    ...
    case "Starlight":
      if (move.hasType("Dark")) {
        addedType = gen.types.get(toID("fairy"));
        typeEffectiveness =
          moveType *
          addedType!.effectiveness[defender.types[0]]! *
          (defender.types[1]
    ...
    case "Underwater":
      addedType = gen.types.get(toID("water"));
      if (
        move.named("Smack Down", "Thousand Arrows") &&
  ...
  if (
    ...
    typeEffectiveness = 1;
    if (
      gen.types.get(toID(move.type))!.effectiveness[defender.types[0]] === 0
    ) {
      typeEffectiveness *= 2;
    } else if (
      gen.types.get(toID(move.type))!.effectiveness[defender.types[0]] === 0.5
    ) {
      typeEffectiveness *= 2;
    } else if (
      gen.types.get(toID(move.type))!.effectiveness[defender.types[0]] === 1
    ) {
      typeEffectiveness *= 1;
    } else if (
      gen.types.get(toID(move.type))!.effectiveness[defender.types[0]] === 2
    ) {
      typeEffectiveness *= 0.5;
    }
    if (defender.types[1] != undefined) {
      if (
        gen.types.get(toID(move.type))!.effectiveness[defender.types[1]] === 0
      ) {
        typeEffectiveness *= 2;
      } else if (
        gen.types.get(toID(move.type))!.effectiveness[defender.types[1]] === 0.5
      ) {
        typeEffectiveness *= 2;
      } else if (
        gen.types.get(toID(move.type))!.effectiveness[defender.types[1]] === 1
      ) {
        typeEffectiveness *= 1;
      } else if (
        gen.types.get(toID(move.type))!.effectiveness[defender.types[1]] === 2
      ) {
        typeEffectiveness *= 0.5;
      }
    }
  ...
  var weightBasedMove = move.named(
    "Heat Crash",
    "Heavy Slam",
    "Low Kick",
    "Grass Knot"
  ...
  var fixedDamage = handleFixedDamageMoves(attacker, move);
  if (fixedDamage) {
  ...
  if (move.named("Spectral Thief")) {
    let stat: keyof typeof defender.boosts;
  ...
  var basePower = calculateBasePowerSMSS(
    gen,
    attacker,
    defender,
    move,
  ...
  var attack = calculateAttackSMSS(
    gen,
    attacker,
    defender,
    move,
  ...
  var attackStat =
    ...
    getShellSideArmCategory(attacker, defender) === "Physical"
      ...
      ? "spa"
      : "atk";
  var defense = calculateDefenseSMSS(
    gen,
    attacker,
    defender,
    move,
  ...
  if (isSpread) {
    baseDamage = pokeRound(OF32(baseDamage * 3072) / 4096); // These might be wrongly put - maybe 4096 need to go in OF32
  }
  ...
  var finalMods = calculateFinalModsSMSS(
    gen,
    attacker,
    defender,
    move,
  ...
  var finalMod = chainMods(finalMods, 41, 131072);
  var childDamage;
  ...
  if (move.dropsStats && move.timesUsed! > 1) {
    ...
    desc.moveTurns = "over ".concat(move.timesUsed?.toString()!, " turns");
    var hasWhiteHerb = attacker.hasItem("White Herb");
    var usedWhiteHerb = false;
    var dropCount = attacker.boosts[attackStat as keyof typeof attacker.boosts];
    var _loop_1 = function (times: number) {
      var newAttack = getModifiedStat(attack, dropCount);
      var damageMultiplier = 0;
      damage = damage.map(function (affectedAmount) {
        if (times) {
          var newBaseDamage = getBaseDamage(
            attacker.level,
            basePower,
            newAttack,
            defense
          );
          var newFinalDamage = getFinalDamage(
            newBaseDamage,
            damageMultiplier,
            typeEffectiveness,
            applyBurn,
      ...
      if (
        hasWhiteHerb &&
        attacker.boosts[attackStat as keyof typeof attacker.boosts] < 0 &&
        !usedWhiteHerb
      ) {
        dropCount += move.dropsStats! * simpleMultiplier;
        usedWhiteHerb = true;
  ...
  desc.attackBoost = move.named("Foul Play")
    ? defender.boosts[attackStat as keyof typeof defender.boosts]
    : attacker.boosts[attackStat as keyof typeof attacker.boosts];
  ...
  if (
    ...
    var lostHP = 0;
    if (
      defender.hasAbility("Damp", "Flash Fire") ||
      field.defenderSide.isProtected
    ) {
      return result;
    } else if (
  ...
  if (
    ...
    if (move.named("Magnitude")) {
      ...
      if (
        attacker.hasAbility("Guts", "Sheer Force", "Huge Power", "Pure Power")
      ) {
        if (attacker.boosts[attackStat as keyof typeof attacker.boosts] > 0) {
          result.damage = lostHP4Over.concat(
            lostHP5Over,
            lostHP6Over,
            lostHP7Over,
            lostHP8Over,
          ...
          result.damage = lostHP4Pow.concat(
            lostHP5Pow,
            lostHP6Pow,
            lostHP7Pow,
            lostHP8Pow,
          ...
          attacker.boosts[attackStat as keyof typeof attacker.boosts] == -1
        ) {
          result.damage = lostHP4Pow.concat(
            lostHP5Pow,
            lostHP6Pow,
          ...
          attacker.boosts[attackStat as keyof typeof attacker.boosts] == -2
        ) {
          result.damage = lostHP4Nice.concat(
            lostHP5Nice,
            lostHP6Nice,
            lostHP7Nice,
            lostHP8Nice,
          ...
          attacker.boosts[attackStat as keyof typeof attacker.boosts] <= -3 &&
          attacker.boosts[attackStat as keyof typeof attacker.boosts] >= -5
        ) {
          result.damage = lostHP4Nice.concat(
            lostHP5Nice,
            lostHP6Nice,
          ...
          attacker.boosts[attackStat as keyof typeof attacker.boosts] == -6
        ) {
          result.damage = lostHP4OK.concat(
            lostHP5OK,
            lostHP6OK,
            lostHP7OK,
            lostHP8OK,
        ...
        if (attacker.boosts[attackStat as keyof typeof attacker.boosts] == 0) {
          result.damage = lostHP4Weak.concat(
            lostHP5Weak,
            lostHP6Weak,
            lostHP7Weak,
            lostHP8Weak,
      ...
      if (
        ...
        if (attacker.boosts[attackStat as keyof typeof attacker.boosts] > 0) {
          ...
          exports.strikerdmg = lostHPPow.concat(
            lostHPPow,
            lostHPPow,
            lostHPPow,
            lostHPOver,
          ...
          exports.strikerdmg.sort(function (a: number, b: number) {
            return a - b;
          });
        } else if (
          attacker.boosts[attackStat as keyof typeof attacker.boosts] == -1
          ...
          exports.strikerdmg = lostHPNice.concat(
            lostHPNice,
            lostHPNice,
            lostHPNice,
            lostHPPow,
          ...
          exports.strikerdmg = lostHPOK.concat(
            lostHPOK,
            lostHPOK,
            lostHPOK,
            lostHPNice,
        ...
        if (attacker.boosts[attackStat as keyof typeof attacker.boosts] == 0) {
          ...
          exports.strikerdmg = lostHPWeak.concat(
            lostHPOK,
            lostHPOK,
            lostHPOK,
            lostHPNice,
          ...
          result.damage = lostHPWeak.concat(
            lostHPOK,
            lostHPNice,
            lostHPPow,
            lostHPOver
          ...
          if (
            ...
            attacker.boosts[attackStat as keyof typeof attacker.boosts] == 4
          ) {
            exports.strikerdmg = lostHPOK.concat(
              lostHPOK,
              lostHPNice,
  ...
  if (move.named("Magnitude")) {
    ...
    exports.magdmg.sort(function (a: number, b: number) {
      return a - b;
    });
    return result;
  }
...
function calculateBasePowerSMSS(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  switch (move.name) {
    ...
    case "Plasma Fists":
      basePower = move.bp;
      if (
        !field.hasTerrain("Underwater", "New World") &&
    ...
    case "Punishment":
      ...
      break;
    case "Low Kick":
      var we = defender.weightkg * getWeightFactor(defender);
      basePower =
        we >= 200
          ? 120
          : we >= 100
      ...
      break;
    case "Grass Knot":
      var w = defender.weightkg * getWeightFactor(defender);
      basePower =
        w >= 200
          ? 120
          : w >= 100
    ...
    case "Natural Gift":
      if (
        (_a = attacker.item) === null || _a === void 0
          ? void 0
          : _a.includes("Berry")
      ) {
        var gift = getNaturalGift(gen, attacker.item!);
  ...
  switch (field.terrain) {
    ...
    case "Burning":
      ...
      if (move.hasType("Fire") && isGrounded(attacker, field)) {
        basePower *= 1.5;
      }
      if (
        (move.hasType("Grass") && isGrounded(defender, field)) ||
        move.hasType("Ice")
      ) {
        basePower *= 0.5;
      }
    ...
    case "Cave":
      if (move.named("Sky Drop")) {
        ...
        move.named("Splintered Stormshards", "Diamond Storm", "Power Gem")
      ) {
        basePower *= 1.3;
      }
    ...
    case "Desert":
      ...
      if (
        (move.hasType("Water") && isGrounded(attacker, field)) ||
        (move.hasType("Electric") && isGrounded(defender, field))
      ) {
        basePower *= 0.5;
      }
      desc.moveBP = basePower;
    ...
    case "Electric":
      ...
      if (isGrounded(attacker, field) && move.hasType("Electric")) {
        basePower *= 1.5;
      }
      desc.moveBP = basePower;
      desc.terrain = field.terrain;
    ...
    case "Grassy":
      if (move.named("Fairy Wind", "Silver Wind")) {
        ...
        move.named("Bulldoze", "Earthquake", "Magnitude", "Muddy Water", "Surf")
      ) {
        basePower *= 0.5;
      }
      if (isGrounded(attacker, field) && move.hasType("Grass")) {
        basePower *= 1.5;
      }
      if (isGrounded(defender, field) && move.hasType("Fire")) {
        basePower *= 1.5;
      }
      if (
        move.named("Sludge Wave", "Acid Downpour") ||
        (move.named(
          "Gust",
          "Hurricane",
          "Razor Wind",
    ...
    case "Murkwater":
      ...
      if (move.hasType("Water")) {
        ...
        basePower *= 1.5;
      } else if (move.hasType("Electric") && isGrounded(defender, field)) {
        basePower *= 1.3;
      }
      desc.moveBP = basePower;
      desc.terrain = field.terrain;
    ...
    case "Psychic":
      ...
      if (isGrounded(attacker, field) && move.hasType("Psychic")) {
        basePower *= 1.5;
      }
      desc.moveBP = basePower;
      desc.terrain = field.terrain;
    ...
    case "Rainbow":
      if (
        ...
        move.named("Splintered Stormshards", "Light That Burns the Sky")
      ) {
        basePower *= 1.3;
      }
    ...
    case "Swamp":
      ...
      if (move.hasType("Poison") && isGrounded(defender, field)) {
        basePower *= 1.5;
      }
      desc.moveBP = basePower;
      desc.terrain = field.terrain;
    ...
    case "Underwater":
      if (move.named("Anchor Shot")) {
        ...
        move.named("Dive", "Bounce", "Fly", "Sky Drop", "Acid Downpour")
      ) {
        basePower *= 1.3;
      }
      if (move.hasType("Fire")) {
    ...
    case "Water":
      ...
      if (move.hasType("Ground")) {
        ...
        basePower *= 1.5;
      } else if (move.hasType("Fire") && isGrounded(defender, field)) {
        basePower *= 0.5;
      }
      desc.moveBP = basePower;
      desc.terrain = field.terrain;
  ...
  var bpMods = calculateBPModsSMSS(
    gen,
    attacker,
    defender,
    move,
...
function calculateBPModsSMSS(
  ...
  if (!resistedKnockOffDamage && defender.item) {
    var item = gen.items.get(toID(defender.item));
...
function calculateAttackSMSS(
  ...
  if (
    attackSource.boosts[attackStat as keyof typeof attackSource.boosts] === 0 ||
    (isCritical &&
      attackSource.boosts[attackStat as keyof typeof attackSource.boosts] < 0)
  ) {
    attack =
      attackSource.rawStats[attackStat as keyof typeof attackSource.rawStats];
  } else if (defender.hasAbility("Unaware")) {
    attack =
      attackSource.rawStats[attackStat as keyof typeof attackSource.rawStats];
    desc.defenderAbility = defender.ability;
  } else {
    attack = attackSource.stats[attackStat as keyof typeof attackSource.stats];
    desc.attackBoost =
      attackSource.boosts[attackStat as keyof typeof attackSource.boosts];
  }
...
function calculateAtModsSMSS(
  ...
  if (
    ...
    if (attacker.hasAbility("Swarm") && field.hasTerrain("Flower Garden 3")) {
      ...
      attacker.hasAbility("Swarm", "Overgrow") &&
...
function calculateDefenseSMSS(
  ...
  desc.defenseEVs = getEVDescriptionText(
    gen,
    defender,
    defenseStat as "atk" | "def" | "spa" | "spd",
    defender.nature
  );
  if (
    defender.boosts[defenseStat as keyof typeof defender.boosts] === 0 ||
    (isCritical &&
      defender.boosts[defenseStat as keyof typeof defender.boosts] > 0) ||
    move.ignoreDefensive
  ) {
    defense = defender.rawStats[defenseStat as keyof typeof defender.rawStats];
  } else if (attacker.hasAbility("Unaware")) {
    defense = defender.rawStats[defenseStat as keyof typeof defender.rawStats];
    desc.attackerAbility = attacker.ability;
  } else {
    defense = defender.stats[defenseStat as keyof typeof defender.stats];
    desc.defenseBoost =
      defender.boosts[defenseStat as keyof typeof defender.boosts];
  }
  if (field.hasWeather("Sand") && defender.hasType("Rock") && !hitsPhysical) {
    defense = pokeRound((defense * 3) / 2);
    desc.weather = field.weather;
  }
  var dfMods = calculateDfModsSMSS(
    gen,
    attacker,
    defender,
    move,
...
function calculateDfModsSMSS(
  ...
  if (field.hasTerrain("New World") && !isGrounded(defender, field)) {
    dfMods.push(3686);
  }
  if (
    (field.hasTerrain("Misty") &&
  ...
  if (
    (defender.hasItem("Eviolite") &&
      ((_a = gen.species.get(toID(defender.name))) === null || _a === void 0
        ? void 0
        : _a.nfe)) ||
    (defender.hasItem("Pikanium Z") &&
      ((_b = gen.species.get(toID(defender.name))) === null || _b === void 0
        ? void 0
        : _b.nfe)) ||
    (defender.hasItem("Light Ball") &&
      ((_c = gen.species.get(toID(defender.name))) === null || _c === void 0
        ? void 0
        : _c.nfe)) ||
...
function calculateFinalModsSMSS(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  var finalMods = [];
  if (
    (attacker.hasAbility("Queenly Majesty") &&
  ...
  if (
    field.hasTerrain("Mountain", "Snowy Mt") &&
    attacker.hasAbility("Long Reach")
  ) {
    finalMods.push(6144);
    desc.defenderAbility = defender.ability;
  ...
  if (
    defender.hasAbility("Flower Veil") &&
