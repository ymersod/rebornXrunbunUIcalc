...
import { magdmg, strikerdmg } from './mechanics/gen78';
...
function getHazards(gen: Generation, defender: Pokemon, defenderSide: Side, field: Field) {
  ...
  if ((defenderSide.isSR && !defender.hasAbility('Magic Guard', 'Mountaineer')) || (field.hasTerrain('Rocky', 'Cave') && defender.hasItem('Telluric Seed') && !defender.hasAbility('Magic Guard', 'Mountaineer'))) {
    var rockType = gen.types.get(toID('rock'))!;
    ...
    if (field.hasTerrain('Crystal Fire')) {
        var rockType = gen.types.get(toID('fire'))!;
        var effectiveness = rockType.effectiveness[defender.types[0]]! *
            (defender.types[1] ? rockType.effectiveness[defender.types[1]] : 1)!;
        damage += Math.floor((effectiveness * defender.maxHP()) / 8);
        ...
        var rockType = gen.types.get(toID('water'))!;
        var effectiveness = rockType.effectiveness[defender.types[0]]! *
            (defender.types[1] ? rockType.effectiveness[defender.types[1]] : 1)!;
        damage += Math.floor((effectiveness * defender.maxHP()) / 8);
        ...
        var rockType = gen.types.get(toID('grass'))!;
        var effectiveness = rockType.effectiveness[defender.types[0]]! *
            (defender.types[1] ? rockType.effectiveness[defender.types[1]] : 1)!;
        damage += Math.floor((effectiveness * defender.maxHP()) / 8);
        ...
        var rockType = gen.types.get(toID('psychic'))!;
        var effectiveness = rockType.effectiveness[defender.types[0]]! *
            (defender.types[1] ? rockType.effectiveness[defender.types[1]] : 1)!;
        damage += Math.floor((effectiveness * defender.maxHP()) / 8);
...
function getEndOfTurn(
  ...
  if ((field.hasTerrain('Swamp') && defender.hasItem('Telluric Seed')) || (field.hasTerrain('Water', 'Murkwater') && defender.hasItem('Elemental Seed')) || (field.hasTerrain('Grassy') && defender.hasItem('Elemental Seed')) || (field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5') && defender.hasItem('Synthetic Seed'))) {
    ...
    texts.push(damage + ' ' + defender.item + ' recovery in ' + field.terrain);
      ...
      damage -= Math.floor(defender.maxHP() / 8);
      texts.push(damage + ' ' + defender.item + ' damage in Wasteland');
  }
  else if (field.hasTerrain('Wasteland')) {
      if (field.defenderSide.spikes > 0 && isGrounded(defender, field)) {
  ...
  else if ((field.hasTerrain('Misty', 'Swamp') && !field.hasWeather('Rain', 'Heavy Rain', 'Harsh Sunshine') && defender.hasAbility('Dry Skin')) || (field.hasTerrain('Water', 'Underwater') && defender.hasAbility('Dry Skin', 'Water Abosrb') && isGrounded(defender, field)) || (field.hasTerrain('Rainbow') && defender.hasStatus('slp')) ||
  ...
  else if (field.hasTerrain('Burning') && isGrounded(defender, field) && !defender.hasType('Fire') && !defender.hasAbility('Flame Body', 'Flare Boost', 'Flash Fire', 'Heatproof', 'Magma Armor', 'Water Bubble', 'Water Veil')) {
      var fireType = gen.types.get(toID('fire'))!;
      ...
      var waterType = gen.types.get(toID('water'))!;
  ...
  else if (field.hasTerrain('Murkwater') && isGrounded(defender, field) && !defender.hasType('Poison', 'Steel') && !defender.hasAbility('Immunity', 'Magic Guard', 'Poison Heal', 'Toxic Boost', 'wonder Guard')) {
      var poisonType = gen.types.get(toID('poison'))!;
  ...
  else if (field.hasTerrain('Corrosive') && defender.hasStatus('slp') && !defender.hasAbility('Magic Guard', 'Immunity', 'Poison Heal', 'Toxic Boost', 'Wonder Guard') && isGrounded(defender, field)) {
      damage -= Math.floor(defender.maxHP() / 16);
  ...
  else if (field.hasTerrain('Murkwater') && defender.hasAbility('Dry Skin', 'Water Absorb') && isGrounded(defender, field)) {
      damage += Math.floor(defender.maxHP() / 8);
  ...
  if (defender.hasStatus('psn')) {
    if (defender.hasAbility('Poison Heal')) {
      if (field.hasTerrain('Corrosive') || (field.hasTerrain('Wasteland', 'Murkwater') && isGrounded(defender, field))) {
          damage += Math.floor(defender.maxHP() / 4);
      }
      else {
          damage += Math.floor(defender.maxHP() / 8);
